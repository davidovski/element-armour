...
public class MetalUtils
{
  ...
  {
    if (checkForHelmet(p)) {
      Block targetBlock = p.getTargetBlock(new HashSet<Material>(), 10);

      if (targetBlock.getType().isSolid()) {
        EvokerFangs f = (EvokerFangs)p.getWorld().spawnEntity(targetBlock.getRelative(BlockFace.UP).getLocation(), org.bukkit.entity.EntityType.EVOKER_FANGS);
        f.setOwner(p);
      } else if (targetBlock.getRelative(BlockFace.DOWN).getType().isSolid()) {
        EvokerFangs f = (EvokerFangs)p.getWorld().spawnEntity(targetBlock.getLocation(), org.bukkit.entity.EntityType.EVOKER_FANGS);
        f.setOwner(p);
      }
    }
  }
  ...
  {
    if ((checkForBoots(p)) &&
      (!p.isOnGround()) && (p.isSneaking())) {
      p.setVelocity(new Vector(0, -1, 0));
      FallingBlock b1 = p.getWorld().spawnFallingBlock(p.getLocation().add(1.0D, 0.0D, 0.0D), Material.IRON_ORE, (byte)0);
      FallingBlock b2 = p.getWorld().spawnFallingBlock(p.getLocation().add(-1.0D, 0.0D, 0.0D), Material.IRON_ORE, (byte)0);
      FallingBlock b3 = p.getWorld().spawnFallingBlock(p.getLocation().add(0.0D, 0.0D, 1.0D), Material.IRON_ORE, (byte)0);
      FallingBlock b4 = p.getWorld().spawnFallingBlock(p.getLocation().add(0.0D, 0.0D, -1.0D), Material.IRON_ORE, (byte)0);
      b1.setVelocity(new Vector(0.0D, 0.5D, 0.0D));
      b2.setVelocity(new Vector(0.0D, 0.5D, 0.0D));
      b3.setVelocity(new Vector(0.0D, 0.5D, 0.0D));
      b4.setVelocity(new Vector(0.0D, 0.5D, 0.0D));
