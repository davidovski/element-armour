    ...
    public void update(LivingEntity e) {
        if ("Werewolf".equals(e.getCustomName())) {
            if (e.getType() == EntityType.WOLF) {
                ...
                wolf.setAngry(true);
                for (Entity a : wolf.getNearbyEntities(20, 20, 20)) {
                ...
                e.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 10, 2));
                ...
                if (Plugin.getMoonPhase(wolf.getWorld()) != MoonPhases.FULL || Plugin.day(wolf.getWorld())) {
                    ...
                    e.remove();
                    vilager.setProfession(Profession.BUTCHER);
                }

            } else {
                if (Plugin.getMoonPhase(e.getWorld()) == MoonPhases.FULL && !Plugin.day(e.getWorld())) {
                    spawn(e.getLocation());
                    e.remove();
                }
            }
        ...
        if (e instanceof Villager) {
            if (((Villager) e).getProfession() == Profession.BUTCHER) {
                if (Plugin.getMoonPhase(e.getWorld()) == MoonPhases.FULL && !Plugin.day(e.getWorld())) {
                    spawn(e.getLocation());
