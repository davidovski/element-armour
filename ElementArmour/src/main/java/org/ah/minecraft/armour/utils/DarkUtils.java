...
public class DarkUtils {
    private static final Color BLACK = Color.fromRGB(0, 1, 0);
    ...
    public static void onClick(PlayerInteractEntityEvent e) {
        if (checkForHelmet(e.getPlayer())) {
            if (e.getRightClicked() instanceof Player) {
                ...
                ((LivingEntity) e.getRightClicked()).addPotionEffect(new PotionEffect(PotionEffectType.WITHER, 200, 2));
            }
        }
    }
    ...
    public static void constantPlayerChecks(Player p) {
        ...
        if (checkForChestplate(p)) {
            p.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 40, 3), true);
            p.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 40, 0), true);
